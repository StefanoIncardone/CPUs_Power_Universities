package com.stefano.gestoreArray;

/**
 * Una libreria per aiutare nella gestione di strutture di tipo array
 */
public abstract class StrumentiArray
{
	/**
	...
	public static final void ruota( int[] array, int offset )
	{
		if( offset > 0 )
		{
			...
			for( int counter = offset; counter < array.length; counter++ )
			{
			...
			{
				array[ counter + array.length - offset ] = temp[ counter ];
			}
		}
	}
}
